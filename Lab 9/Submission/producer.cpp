...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        cerr << "Usage: " << argv[0] << " <file_path> <fps>" << endl;
        return 1;
    }

    string file_path = argv[1];
    int fps = stoi(argv[2]); // argv[2] is the target FPS for the producer

    if (fps <= 0) {
        cerr << "FPS must be a positive integer." << endl;
        return 1;
    }
    ...
    int sleep_ms = 1000 / fps; // Sleep duration in milliseconds between frame productions, derived from FPS

    while (running) {

        // Loop through the pre-loaded frames
